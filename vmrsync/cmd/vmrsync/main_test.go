...
func TestMissingMachine(t *testing.T) {
	_, code := runBinary([]string{"restore"})
	if code == 0 {
...
func TestDryRun(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpSrc, "patterns.txt"), []byte("*.tmp\n"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			args:    []string{"--dry-run", "restore", "vm21", "project1"},
			env:     []string{srcEnv},
			wantOut: "rsync -az",
		},
		{
			name:    "dry-run shows rsync command",
			args:    []string{"restore", "vm21", "project1", "--dry-run"},
			env:     []string{srcEnv},
			wantOut: "rsync -az",
		},
		{
			name:    "ssh port passed to rsync",
			args:    []string{"restore", "vm21", "project1", "--dry-run", "--ssh-port", "2222"},
			env:     []string{srcEnv},
			wantOut: "-p 2222",
		},
		{
			name:    "ssh key passed to rsync",
			args:    []string{"backup", "vm21", "project1", "--dry-run", "--ssh-key", "/tmp/id_rsa"},
			env:     []string{srcEnv},
			wantOut: "-i /tmp/id_rsa",
		},
		{
			name:    "verbose adds -v flag",
			args:    []string{"restore", "vm21", "project1", "--dry-run", "--verbose"},
			env:     []string{srcEnv},
			wantOut: " -v ",
		},
		{
			name:       "no-delete omits --delete",
			args:       []string{"backup", "vm21", "project1", "--dry-run", "--no-delete"},
			env:        []string{srcEnv},
			wantNotOut: "--delete",
		},
		{
			name:    "default includes --delete",
			args:    []string{"backup", "vm21", "project1", "--dry-run"},
			env:     []string{srcEnv},
			wantOut: "--delete",
		},
		{
			name:    "multiple excludes",
			args:    []string{"backup", "vm21", "project1", "--dry-run", "--exclude", "*.log", "--exclude", "node_modules"},
			env:     []string{srcEnv},
			wantOut: "--exclude=node_modules",
		},
		{
			name:    "default mirrors local root (no folder)",
			args:    []string{"restore", "vm21", "--dry-run"},
			env:     []string{srcEnv},
			wantOut: "vm21:" + tmpSrc + "/",
		},
		{
		...
		},
		{
			...
			args:    []string{"backup", "vm21", "71", "--dry-run", "--staging"},
			env:     []string{srcEnv},
			wantOut: "vm21:/vmrsync/71/",
		},
		{
			name:       "default does not use /vmrsync",
			args:       []string{"backup", "vm21", "71", "--dry-run"},
			env:        []string{srcEnv},
			wantNotOut: "/vmrsync",
		},
		{
			name:    "exclude-from patterns",
			args:    []string{"backup", "vm21", "project1", "--dry-run", "--exclude-from", filepath.Join(tmpSrc, "patterns.txt")},
			env:     []string{srcEnv},
			wantOut: "--exclude=*.tmp",
		},
		{
			name:    "prepare dry-run",
			args:    []string{"prepare", "vm21", "--dry-run"},
			env:     []string{"VMRSYNC_SKIP_UID_CHECK=1"},
			wantOut: "ssh",
		},
	}
...
func TestRunSyncStaging(t *testing.T) {
	...
	rsyncrun.ExecSSH = func(args []string) error { return nil }
...
func TestBuildSSHFlags(t *testing.T) {
	base := []string{"-o", "BatchMode=yes", "-o", "ConnectTimeout=10"}
	tests := []struct {
		port string
		key  string
		want []string
		...
		{"", "", base},
		{"2222", "", append(append([]string{}, base...), "-p", "2222")},
		{"", "/tmp/id_rsa", append(append([]string{}, base...), "-i", "/tmp/id_rsa")},
		{"2222", "/tmp/id_rsa", append(append(append([]string{}, base...), "-p", "2222"), "-i", "/tmp/id_rsa")},
	}
	for _, tt := range tests {
...
func TestParseArgs(t *testing.T) {
	tests := []struct {
		...
		{
			osArgs:      []string{"vmrsync", "restore", "vm21"},
			command:     "restore",
			wantMachine: "vm21",
			wantTimeout: 7200,
		},
		{
			osArgs:      []string{"vmrsync", "restore", "vm21", "proj"},
			command:     "restore",
			wantMachine: "vm21",
			wantFolder:  "proj",
			wantTimeout: 7200,
		...
		{
			osArgs:      []string{"vmrsync", "restore", "vm21", "--dry-run"},
			command:     "restore",
			wantMachine: "vm21",
			wantDryRun:  true,
			wantTimeout: 7200,
		...
		{
			osArgs:      []string{"vmrsync", "backup", "vm21", "proj", "--ssh-port", "2222"},
			command:     "backup",
			wantMachine: "vm21",
			wantFolder:  "proj",
			wantPort:    "2222",
		...
		{
			osArgs:      []string{"vmrsync", "restore", "vm21", "--ssh-key", "/tmp/id_rsa"},
			command:     "restore",
			wantMachine: "vm21",
			wantKey:     "/tmp/id_rsa",
			wantTimeout: 7200,
		...
		{
			osArgs:       []string{"vmrsync", "restore", "vm21", "--verbose", "--no-delete"},
			command:      "restore",
			wantMachine:  "vm21",
			wantVerbose:  true,
			wantNoDelete: true,
		...
		{
			osArgs:      []string{"vmrsync", "restore", "vm21", "a/b/c"},
			command:     "restore",
			wantMachine: "vm21",
			wantFolder:  "a/b/c",
			wantTimeout: 7200,
		...
		{
			osArgs:      []string{"vmrsync", "backup", "vm21", "--timeout-seconds", "0"},
			command:     "backup",
			wantMachine: "vm21",
			wantTimeout: 0,
		},
...
func TestCheckRemoteDirExists(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		joined := strings.Join(capturedArgs, " ")
		for _, want := range []string{"BatchMode=yes", "-p 2222", "-i /tmp/id_rsa", "vm21", "test -d"} {
...
func TestRunPrepare(t *testing.T) {
	t.Setenv("VMRSYNC_SKIP_UID_CHECK", "1")
	...
	t.Run("executes ssh with remote mkdir", func(t *testing.T) {
		...
		prepare.Run([]string{"vm21"})

		joined := strings.Join(capturedArgs, " ")
		uid := fmt.Sprintf("%d", os.Getuid())
		gid := fmt.Sprintf("%d", os.Getgid())
		wantChown := fmt.Sprintf("chown %s:%s", uid, gid)
		for _, want := range []string{"vm21", "sudo mkdir -p", wantChown} {
			if !strings.Contains(joined, want) {
	...
	t.Run("dry-run does not call ssh", func(t *testing.T) {
		...
		prepare.Run([]string{"vm21", "--dry-run"})
	...
	t.Run("ssh port and key forwarded", func(t *testing.T) {
		...
		prepare.Run([]string{"vm21", "--ssh-port", "2222", "--ssh-key", "/tmp/id_rsa"})
...
func TestRunSyncBackup(t *testing.T) {
	var capturedArgs []string
	rsyncrun.ExecSSHCheck = func(args []string) error { return nil }
	rsyncrun.ExecSSH = func(args []string) error { return nil }
	rsyncrun.ExecRsync = func(ctx context.Context, args []string) error {
...
func TestRunSyncMkpathFallbackBackup(t *testing.T) {
